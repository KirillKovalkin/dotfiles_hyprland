...
Scope {
    ...
    IpcHandler {
        ...
        function toggle(): void {
            ...
            if (root.panelVisible) {
                // Data setup — safe immediately (no panel ids needed)
                root.searchText = ""
                root.selectedIndex = 0
                // UI setup (forceActiveFocus, scroll) deferred to Loader.onLoaded
            } else {
    ...
    onSelectedIndexChanged: {
        ...
        const item = themePanelLoader.item
        if (!item) return
        if (filteredThemes.length > 0
            ...
            item.themeListView.positionViewAtIndex(selectedIndex, ListView.Contain)
        }
    }
    ...
    // ── Panel loaded on demand — saves ~270 lines of instantiation at startup ──
    Loader {
        id: themePanelLoader
        ...
        onLoaded: {
            const item = themePanelLoader.item
            if (!item || !root.panelVisible) return
            // Post-load UI setup
            item.searchInputField.text = ""
            item.searchInputField.forceActiveFocus()
            ...
            item.themeListView.positionViewAtIndex(root.selectedIndex, ListView.Center)
        }
    }
    ...
    Component {
        ...
        PanelWindow {
            id: themePanel
            // Expose inner ids for Scope-level handlers (onSelectedIndexChanged, IpcHandler)
